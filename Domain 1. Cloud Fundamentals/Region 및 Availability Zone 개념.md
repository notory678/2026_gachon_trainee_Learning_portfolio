...
### 📍 [사용 가능한 리전 현황](https://docs.kakaocloud.com/start/region-az#%EC%82%AC%EC%9A%A9-%EA%B0%80%EB%8A%A5%ED%95%9C-%EB%A6%AC%EC%A0%84)
| 대상 고객 | 리전 코드 | 위치 | 가용 영역(AZ) |
| :--- | :--- | :--- | :--- |
| **일반/기업 고객용** | `kr-central-2` | 수도권 | a, b, c (d 예정) |
| **공공기관 고객용** | `kr-gov-central-2` | 수도권 | a |
...
> **Tip:** 리소스를 여러 리전에 배치하면 더욱 높은 수준의 보안을 달성할 수 있습니다. 각 리소스에 대한 자세한 설명은 [리소스 구분](https://docs.kakaocloud.com/start/resource)을 참고하시기 바랍니다.

# 3. [Scenario flow] 

## 📖 VPC,서브넷 생성과 리소스 배치시 리전, AZ 선택 방법
...
```mermaid
  graph LR
    %% 노드 정의
    A(시작: 리전 선택) 
    B(VPC 생성) --> C(AZ 선택 및 서브넷 생성)
    D(리소스 배치: BCS/DB 생성) --> E(물리적 위치 확정)

    %% 설명 및 제약사항 추가 (주석 형태)
    subgraph P1 [Phase 1: 물리적 거점 확정]
    A
    end

    subgraph P2 [Phase 2: 네트워크 구획 설계]
    B
    C
    end

    subgraph P3 [Phase 3: 실무 자원 할당]
    D
    E
    end
